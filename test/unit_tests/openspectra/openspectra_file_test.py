...
class OpenSpectraHeaderTest(unittest.TestCase):
    ...
    def test_file_parse_sample_one(self):
        ...
        with self.assertLogs("openSpectra.OpenSpectraHeader", level='INFO') as log:
            ...
            self.assertEqual(map_info.x_reference(), 1.0)
            self.assertEqual(map_info.y_reference(), 1.0)
            self.assertEqual(map_info.x_coordinate(), 50000.0)
            self.assertEqual(map_info.y_coordinate(), 4000000.0)
